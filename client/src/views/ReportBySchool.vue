...
const groupedStudents = computed(() => {
  ...
  for (let x of journal.value) {
    ...
    data[x.students].lessons[x.lessons_name][unformatDate(x.date)] = x.mark;
  }
  console.log(data);
  return data;
});
...
async function fetchReports(subjectId) {
  ...
  header.push("ФИО");
  headeralias.push("user");
  header.push("Предмет");
  headeralias.push("suj");
  ...
  for (var i = 0; i < header.length; i++) {
    data.columns.push({ label: header[i], value: headeralias[i] });
  }
  //forming content
  for (let key0 in groupedStudents.value) {
    for (let key1 in groupedStudents.value[key0].lessons) {
      let currlist = {
        [headeralias[0]]: groupedStudents.value[key0].students_name,
        [headeralias[1]]: key1,
      };
  ...
  data = [data];
  let ectitle =
    document.getElementById("selececol").options[
      document.getElementById("selececol").selectedIndex
    ].text;
  let settings = {
...
function downloadXLSX() {
  let ec = document
    .getElementById("selececol")
    .options[document.getElementById("selececol").selectedIndex].value.toString(
      10
    );
  fetchReports(ec);
}
