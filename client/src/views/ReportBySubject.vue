...
function downloadxlsx() {
  ...
  for (
    var i = 0;
    i < document.getElementById("head").childElementCount - 1;
    i++
  ) {
  ...
  for (var i = 1; i < document.getElementById("jour").childElementCount; i++) {
    ...
    for (var j = 0; j < header.length; j++) {
      var ind = currstr.indexOf("\n");
      jsonData =
        jsonData + ' "' + header[j] + '" : "' + currstr.slice(0, ind) + '"';
  ...
  let settings = {
    fileName:
      "Отчёт по предмету " +
      document.getElementById("selecsuj").options[
        document.getElementById("selecsuj").selectedIndex
      ].text, // Name of the resulting spreadsheet
